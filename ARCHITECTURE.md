...
- `src/components/message`：用户/助手消息、Markdown、代码块、思考过程、工具调用树；助手正文与思考过程复用同一安全 Markdown 渲染器，思考过程保持紧凑字号并关闭 artifact 预览。Markdown 使用 GFM、数学公式和受控代码/图表预览；任务列表保持只读，脚注提供返回链接，外部链接新窗口打开，Markdown 图片复用现有 ImageLightbox。Chat 与 Document 当前都保留 `remark-breaks` 的硬换行契约，以免提取表格和段落中的显式换行被折叠；代码高亮按需加载已登记的常见语言，未知 fence 继续安全回退纯文本。
...
模型目录的持久事实仍由 PostgreSQL `models` 表拥有；backend 启动会用该表整体替换进程内 registry，空表也不会回退到编译期内置型号。migration `046_model_catalog_lifecycle.sql` 为每条管理员模型记录增加 `catalog_source`、`catalog_checked_at` 和 `lifecycle_status`：既有与普通手工创建记录默认是 `manual` / `unknown`，渠道 `/models` 与 models.dev 只产生默认 disabled 的候选并携带各自来源及核对时间。目录中出现一个 ID 不等于账号可运行，因此除显式 preview/experimental 标记外不从名称猜测 active/deprecated/retired；管理员记录不会因目录消失而被自动删除。**管理员显式 DELETE 是独立的硬删除操作，直接移除 `models` 行；启用/禁用仍由独立的 enabled mutation 负责。硬删除后历史消息仍保留其 model_id 字符串，但新的模型目录和 registry 不得重新显示该数据库记录。** Admin 模型列表和编辑器显示来源、核对时间与 lifecycle；从渠道导入或应用 models.dev 能力时保留对应来源，管理员手工修改 capability/lifecycle 时切换为 `manual` 并清除旧目录核对时间，显示名称、可见组和排序等非能力字段不改变来源。migration `047_model_temperature_profile.sql` 以 `configurable`、`omit`、`fixed` 三种 typed policy 持久化 temperature 请求契约；固定策略要求 0–2 的显式值，其他策略禁止残留值。migration `048_model_openai_request_profile.sql` 进一步为 OpenAI-compatible 模型提供可空且有范围约束的 `top_p`、`n`、`presence_penalty`、`frequency_penalty` 固定值；空值代表完全省略字段，Admin 不接受任意 provider 参数 JSON。统一 ChatModel 构造在 accepted runtime snapshot 中固化这些模型级请求约束：temperature 的 `configurable` 保留会话值，`omit` 传 `nil`，`fixed` 使用管理员值；OpenAI-compatible typed profile 只在对应 adapter 边界映射为 wire 字段。Google native 进一步按已核验的 Gemini generation contract 映射请求：Gemini 2.5 使用 numeric `thinkingBudget`，Gemini 3.0/3.1/3.5/3.6/3.7 使用 typed `thinkingLevel` 并省略已不接受的采样参数；未知未来版本不继承猜测字段。不能关闭 thinking 的 Gemini 3.x utility 请求使用最低合法 level，避免省略配置后回到 provider 默认档位。Anthropic native 对 Claude 4.6+、Claude 5 与 Fable/Mythos 使用 adaptive thinking：用户对话显式请求 `display=summarized`，effort 选项按型号开放 `low/medium/high/xhigh/max`；utility 使用最低合法 effort 且省略 display，避免 reasoning 摘要进入结构化任务输出。DeepSeek V4 的 OpenAI-compatible profile 使用 `thinking.type` 与 `reasoning_effort=low/high/max`；中间档位按官方兼容语义归一到 high，utility 显式关闭 thinking，同一轮工具 continuation 保留 assistant `reasoning_content` 与原 tool call ID。历史 `deepseek-chat` / `deepseek-reasoner` 不再由编译期兜底目录恢复，但数据库既有行仍由管理员拥有。Qwen 3.7 Max/Plus 继续走 OpenAI-compatible `enable_thinking + thinking_budget`：hybrid 型号可显式关闭，thinking-only 型号的 utility 使用最小预算，只有已核验的 3.6/3.7 请求发送 `preserve_thinking=true`。`SearchImplParams` 在该 wire 边界映射为 `enable_search=true`，Google native 仍独立映射为 `google_search`。GLM-5.2/5.3 在 OpenAI-compatible wire 上使用 `thinking.type` 与 `reasoning_effort=low/high/max`；5.2 可关闭，5.3 固定开启且 stale `none` 或 utility 请求都归一为最低合法 low，旧 GLM-4.5 保持 toggle-only。MiniMax model family 在 adapter 之前解析：M3 在 OpenAI- 和 Anthropic-compatible 通道都使用 `thinking.type=adaptive/disabled`，M2.7 固定开启且不发送控制字段；两个 adapter 仍各自保留原生 reasoning 内容投影，不自动跨协议 fallback。Kimi 同样由 model family 决定而不是按 `moonshot` provider 猜字段：K3 使用 `reasoning_effort=low/high/max`、`max_completion_tokens` 并省略固定采样参数，K2.7 Code 固定思考且不发送控制字段，K2.6 使用 `thinking.type=enabled/disabled`，开启时带 `keep=all`；utility 分别归一到 low、保持无控制字段或显式 disabled。未知 Moonshot alias 不获得这些推断。xAI standard reasoning 继续走 OpenAI-compatible Chat Completions，但按型号开放 effort：Grok 4.6、4.20 与 4.1 Fast 允许 `xhigh`，旧型号保持 `low/medium/high`；multi-agent 与显式 non-reasoning 型号不套用该 profile。Grok utility 请求使用最低合法 `low`，请求构造会省略该家族不接受的 presence/frequency penalty，而不修改管理员保存的 typed profile。主对话、压缩、记忆维护、网页 refinement 与最小 probe 复用同一解析，不修改会话中保存的用户偏好。发布候选日只在官方目录与受控 models.dev 同时给出一致 ID/capability 时更新编译期保守兜底；该更新仍不 seed 数据库、不自动启用或替换管理员模型。管理员 partial update 在 repository transaction 内先 `SELECT ... FOR UPDATE` 重读 canonical row，再只把请求携带的字段应用到该行并校验跨字段约束；不同字段的并发请求因此可以顺序合成，禁用模型也复用窄 `enabled` patch，不再把 service 层旧快照全字段写回。
...
管理员 User Group list/create/update/delete 复用稳定资源错误边界：ID、名称、描述、等级与配额限制校验为 400，名称重名及撤销/删除最后默认组的 invariant 冲突为 409，缺失资源为 404，repository/transaction 故障为带 request ID 的 retryable 5xx。production migration 为 fresh install 建立一个 level 0、无限额的 `Default` 组；已有默认组的部署不改动，只有尚无默认组且已存在同名组时才把该组设为默认而保留其等级与配额。默认组 advisory-lock 与事务保护继续由 repository 持有。partial update 在取得该 advisory lock 和目标 row lock 后重读 canonical group、应用请求携带的字段并校验完整配额与默认组 invariant；不同字段并发顺序合成，同字段采用明确的事务顺序 last-write-wins，不再把锁外旧对象全字段写回。`users.group_id` 是原始显式绑定；NULL 不复制某个历史组，也不等于 level 0，而是由 repository 在每次权限或配额读取时动态解析为当前唯一默认组。模型与 Skill 消费共享的 effective level，quota 消费同一 effective group 的限制字段；默认组切换、等级/限额更新及显式组删除后的 `ON DELETE SET NULL` 因而立即在各链一致生效。
...
聊天正文的中文与 Latin 字体 face 使用互斥 `unicode-range` 建立字形所有权：CJK/Han/Kana/Hangul/full-width 区间由中文槽位负责，ASCII、Latin 扩展与常用西文符号由英文槽位负责，因此同时包含 ASCII 的 CJK 字体不会遮蔽英文配置。未配置管理员字体时，英文优先使用 Plus Jakarta Sans，中文按平台优先回退到 Apple 系统字体、Segoe UI/Microsoft YaHei、PingFang SC，Noto Sans SC 作为 web fallback，以减少官网对比场景下的字面疏松和跨平台度量差异；聊天消息列使用标准/紧凑桌面共享最大宽度 token，回答块的桌面上下节奏略收紧，移动端保留原有触摸与阅读留白；代码槽位保持独立 `--chat-code-font-family` 且不加字符范围，以支持源码中的 Unicode 标识符、字符串和注释。该路由只改变浏览器 glyph 选择和消息列几何，不改变字体文件、槽位持久化、显式 null 或生命周期事务。

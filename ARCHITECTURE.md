...
模型目录的持久事实仍由 PostgreSQL `models` 表拥有；backend 启动会用该表整体替换进程内 registry，空表也不会回退到编译期内置型号。migration `046_model_catalog_lifecycle.sql` 为每条管理员模型记录增加 `catalog_source`、`catalog_checked_at` 和 `lifecycle_status`：既有与普通手工创建记录默认是 `manual` / `unknown`，渠道 `/models` 与 models.dev 只产生默认 disabled 的候选并携带各自来源及核对时间。目录中出现一个 ID 不等于账号可运行，因此除显式 preview/experimental 标记外不从名称猜测 active/deprecated/retired；管理员记录不会因目录消失而被删除，后续 request profile 与候选日差异门禁在独立切片收口。

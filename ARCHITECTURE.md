...
聊天正文的中文与 Latin 字体 face 使用互斥 `unicode-range` 建立字形所有权：CJK/Han/Kana/Hangul/full-width 区间由中文槽位负责，ASCII、Latin 扩展与常用西文符号由英文槽位负责，因此同时包含 ASCII 的 CJK 字体不会遮蔽英文配置。未配置管理员字体时，正文与 Composer 回退到现有 `--font-sans` 系统无衬线栈，以减少 Windows/macOS 字体度量和字形风格差异；代码槽位保持独立 `--chat-code-font-family` 且不加字符范围，以支持源码中的 Unicode 标识符、字符串和注释。该路由只改变浏览器 glyph 选择，不改变字体文件、槽位持久化、显式 null 或生命周期事务。

...
- 备份入口不会复制在线 PGDATA：它优雅停止当前正在运行的 Web/backend/提取器，避免把跨数据库与文件系统的在途事务冻结在中间状态；随后在同一静止窗口生成 PostgreSQL custom-format dump 与 storage tar，并在版本、build ref、schema、PostgreSQL major、工件 SHA-256 和逐文件清单齐全后原子发布，最后只恢复原先运行的服务；`.env.docker` 和 secret 不进入备份。

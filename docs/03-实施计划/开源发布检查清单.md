...
>
> 当前基线：0.3.4-beta.1 release candidate；tag 尚未创建。
...
- [x] 三个应用镜像先构建为 commit staging manifest；全部成功后才统一晋级版本与 `sha-` tag，并生成 SBOM 与 provenance。
- [ ] 公开 `main` 已启用分支保护，并要求 CI 与 Gitleaks 通过。
- [ ] GitHub 私密漏洞报告已启用。
- [ ] 仓库描述、topics 和可见性已核对。
...
- [ ] 镜像 build ref、SBOM 和 provenance 可追溯到 tag。
- [ ] 预发布 tag 没有更新 `latest`。

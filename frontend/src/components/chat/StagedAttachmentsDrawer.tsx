  ...
  return (
    <div className="rounded-md border border-border/70 bg-muted/15 px-2.5 py-2">
      <div className="flex items-center gap-2">
        <div className="min-w-0 flex-1"><div className="truncate text-xs font-medium">{task.filename}</div><div className="truncate text-xs text-muted-foreground">{label}</div></div>
  ...
  return (
    <div className={cn("group relative min-w-0 overflow-hidden rounded-lg border shadow-sm transition-[border-color,box-shadow,opacity] motion-control hover:shadow-md", selectedOrder ? "border-primary/60 bg-primary/5" : "border-border/70 bg-muted/15 hover:border-border", blocked && "opacity-60")}>
      <button type="button" disabled={blocked} onClick={onToggle} className="block w-full text-left outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring disabled:cursor-not-allowed" aria-label={`${selectedOrder ? "取消选择" : "选择"}附件：${file.filename}`}>
        ...
        <div className="min-w-0 px-2 py-1.5"><div className="truncate text-xs font-medium">{file.filename}</div><div className={cn("truncate text-xs text-muted-foreground", blocked && "text-amber-600")}>{status || formatBytes(file.size)}</div></div>
      </button>
      <button type="button" onClick={onPreview} className="absolute inset-x-1 bottom-[3.4rem] flex h-7 items-center justify-center rounded-md bg-background/80 text-xs text-foreground opacity-100 shadow-sm backdrop-blur-sm transition-[background-color,opacity] motion-control hover:bg-background sm:opacity-0 sm:focus:opacity-100 sm:group-hover:opacity-100" aria-label={`预览附件：${file.filename}`}>预览</button>
      {selectedOrder ? <span className="absolute left-1.5 top-1.5 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-primary px-1.5 text-xs font-medium text-primary-foreground shadow-sm">{selectedOrder}</span> : null}
